$(function() {
	...
	$(document).on('click', '.payOnline', function(e) {
		...
		createModal('在线支付', 'myOrderPayOnline', 'myOrderPaysave()');
		return false;
	});
	//白条支付modal
	$(document).on('click', '.payCredit', function(e) {

		createModal('白条支付', 'myOrderPayCredit', 'myOrderPayOnlinesave()');
		return false;
	});
	//填提货函modal
	$(document).on('click', '.pickLetter', function(e) {
		var orderNo = $(this).attr('data-orderNo');
		createModal('填提货函', 'myOrderPickLetter?orderNo=' + orderNo, 'myOrderPickLettersave()');
		return false;
	});
	//确认收货
	$(document).on('click', '.confirm', function(e) {
		var orderNo = $(this).attr('data-orderNo');
		$.post(app.approot + "www/member/myOrderConfirm", {
			orderNo: orderNo
		}, function(data) {
	...
	$(document).on('click', '.received', function(e) {
		var orderNo = $(this).attr('data-orderNo');
		$.post(app.approot + "www/member/myOrderReceived", {
			orderNo: orderNo
		}, function(data) {
			if(data.status == '0' && data.content) {
				getList();
	...
	$(document).on('click', '.evaluate', function(e) {
		var orderNo = $(this).attr('data-orderNo');
		createModal('评价', 'myOrderPayEvaluate?orderNo=' + orderNo, 'myOrderEvaluatesave()');
		return false;
	});
	...
	$.ajax({
		...
		success: function(data) {
			if(data.status === 0) {
				...
				$("#tMain").html(html);
				console.dir(data.content.content);
	            //增加付款倒计时
...
function getCounts() {
	$.ajax({
		...
		success: function(data) {
			...
			var zong = parseInt(_content[1])+parseInt(_content[2])+parseInt(_content[3])+parseInt(_content[4])+parseInt(_content[5])+parseInt(_content[6]);
			$('#span1').html(zong);
			$('#span2').html(_content[0]);
			$('#span3').html(_content[1]);
			$('#span4').html(_content[2]);
			$('#span5').html(_content[3]);
			$('#span6').html(_content[6]);
			$('#span7').html(_content[5]);
		}
	});
}
